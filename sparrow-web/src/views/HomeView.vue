...
import AiDock from '../components/AiDock.vue'
import LoginModal from '../components/LoginModal.vue'
import MemberModal from '../components/MemberModal.vue'
import { fetchTree, fetchNode, fetchPrerequisites } from '../api/graph'
import type { Tree, NodeBrief, NodeDetail } from '../api/graph'
...
function layout(nodes: NodeBrief[]) {
...
function buildOption() {
  ...
  const eraLabels = Object.entries(eras).map(([rank, eraName]) => {
    const r = Number(rank)
    const ys = nodes.filter((n) => n.eraRank === r).map((n) => pos[n.id].y)
    return {
      id: `era-${rank}`,
      name: eraName,
      x: (r - 1) * COL_W,
  ...
  const data = nodes.map((n) => {
    ...
    eraLabels.map((e) => ({ ...e, _nodeId: null as number | null }))
  )

  const edgeData = edges.map((e) => {
    const active =
      chain && (chain.has(e.from) || e.from === selected) && (chain.has(e.to) || e.to === selected)
    return {
...
async function showNode(id: number) {
  ...
  highlight.value = {
    ...
    chainIds: new Set(chain.map((n) => n.id)),
  }
