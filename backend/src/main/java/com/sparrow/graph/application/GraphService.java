...
import com.sparrow.graph.api.dto.GraphDtos.EdgeBrief;
...
@Service
public class GraphService implements GraphQueryService, TechNodeCatalog {
    ...
    public List<IndexableNode> listIndexableNodes() {
        return nodeMapper.selectList(null).stream()
                .sorted(Comparator.comparing(TechNode::getEraRank).thenComparing(TechNode::getId))
                .map(n -> new IndexableNode(n.getId(), n.getName(), n.getEra(), n.getYearLabel(),
                        n.getSummary(), n.getDetail()))
                .toList();
    }
}
