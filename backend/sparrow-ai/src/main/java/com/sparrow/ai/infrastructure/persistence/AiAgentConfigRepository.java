...
@Repository
public class AiAgentConfigRepository {

    private static final String SERVICE = "sparrow-ai";
    ...
    @PostConstruct
    void initSchema() {
        jdbc.execute("CREATE TABLE IF NOT EXISTS ai_agent_config ("
        ...
        jdbc.execute("CREATE TABLE IF NOT EXISTS ai_agent_config_audit ("
                ...
                + "KEY idx_ai_agent_audit_key(agent_key,id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    }

    public void ensureDefault(AiAgentProfile profile) {
        jdbc.update("INSERT IGNORE INTO ai_agent_config(agent_key,display_name,description,system_prompt,"
    ...
    public List<AiAgentProfile> list() {
        return jdbc.query("SELECT agent_key,display_name,description,system_prompt,enabled,"
                + "max_context_messages,max_context_chars,max_output_chars,max_steps,updated_by,updated_at "
                + "FROM ai_agent_config ORDER BY agent_key", (rs, row) -> map(rs));
    }

    public Optional<AiAgentProfile> find(String agentKey) {
        List<AiAgentProfile> rows = jdbc.query("SELECT agent_key,display_name,description,system_prompt,enabled,"
                        + "max_context_messages,max_context_chars,max_output_chars,max_steps,updated_by,updated_at "
                        + "FROM ai_agent_config WHERE agent_key=?",
    ...
    public int update(String agentKey, String systemPrompt, boolean enabled,
        ...
        return jdbc.update("UPDATE ai_agent_config SET system_prompt=?,enabled=?,max_context_messages=?,"
    ...
    public void audit(String agentKey, long operatorId, String summary) {
        jdbc.update("INSERT INTO ai_agent_config_audit(agent_key,operator_id,action,summary) VALUES(?,?,?,?)",
    ...
    public List<AuditRow> audits(int limit) {
        return jdbc.query("SELECT id,agent_key,operator_id,action,summary,created_at "
                        + "FROM ai_agent_config_audit ORDER BY id DESC LIMIT ?",
